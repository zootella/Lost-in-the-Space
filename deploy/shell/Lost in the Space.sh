...
# Type "./Thisfile.sh" on the command line to launch the Java program
# Didn't work? Try "chmod 777 Thisfile.sh" first

# Change the directory in case we were started from a graphical shell like Nautilus
cd "`dirname "$0"`"
cd "Lost in the Space.app/Contents/Resources/Java"
...
MSG7="You don't seem to have a valid JRE. Get the Sun JRE at http://www.java.com "
...
if [ $? -ne 0 ]; then
  echo 
  echo "Something went wrong starting the program."
  echo "Maybe you're using the wrong version of Java?"
  echo "Sun's JRE, Java 1.6 or later, is required."
